@if(isset($users_paginate))
    <!-- resources/views/custom-pagination.blade.php -->

    @if ($users_paginate->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between bg-gray-100">
            <ul class="pagination flex space-x-4 px-4 w-[100%] justify-center items-center">
                ...
                {{-- Page Number Links --}}
                @php
                    $currentPage = $users_paginate->currentPage();
                    $lastPage = $users_paginate->lastPage();
                ...
                {{-- Next Page Link --}}
                @if ($users_paginate->hasMorePages())
                ...
                <!-- <div class="text-sm text-gray-500 leading-5">
                    Showing {{ $users_paginate->firstItem() }} to {{ $users_paginate->lastItem() }} of {{ $users_paginate->total() }} results
                </div> -->
            </ul>
        </nav>
    @endif
