@if(isset($checklists_paginate))
    <!-- resources/views/custom-pagination.blade.php -->

    @if ($checklists_paginate->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between bg-gray-100">
            <ul class="pagination flex space-x-4 px-4 w-full justify-center items-center">
                {{-- Previous Page Link --}}
                @if ($checklists_paginate->onFirstPage())
                ...
                @else
                    <a href="{{ $checklists_paginate->previousPageUrl() }}" rel="prev" class="absolute left-0 inline-flex items-center p-2 text-sm font-medium text-gray-500 border-t-2 border-gray-300 hover:bg-rose-100 hover:border-rose-600 hover:text-rose-600 focus:outline-none focus:ring ring-rose-300 focus:border-rose-300 active:text-rose-700 transition ease-in-out duration-150" aria-label="@lang('pagination.previous')">
                        <span class="flex items-center">
                            <i class='bx bx-left-arrow-alt text-[18px]'></i>ก่อนหน้า
                        </span>
                    </a>
                ...
                {{-- Page Number Links --}}
                @php
                    $currentPage = $checklists_paginate->currentPage();
                    $lastPage = $checklists_paginate->lastPage();
                ...
                <div class="flex inline">
                    ...
                    @foreach (range($start, $end) as $page)
                        ...
                        @else
                            <li>
                                <a href="{{ $checklists_paginate->url($page) }}" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 border-t-2 border-gray-300 hover:text-rose-600 hover:border-rose-600 focus:outline-none focus:ring ring-rose-300 focus:border-rose-300 active:border-rose-100 active:text-rose-700 transition ease-in-out duration-150">
                                    {{ $page }}
                                </a>
                            </li>
                        @endif
                ...
                <!-- <div class="text-sm text-gray-500 leading-5">
                    Showing {{ $checklists_paginate->firstItem() }} to {{ $checklists_paginate->lastItem() }} of {{ $checklists_paginate->total() }} results
                </div> -->
            </ul>
        </nav>
    @endif
