...
@section('ChecklistSelected')
    <section class="h-full relative grow bg-[#CDD4E8] ">
        <!-- Header -->
        <div class="p-4 flex flex-col my-auto mb-[-4px] max-md:hidden">
            <div class="flex items-center pl-5">
                ...
                <a href="{{ Route('Checklist') }}" class="ml-4 font-medium text-[20px] text-gray-600 underline whitespace-nowrap hover:text-black duration-300">รายการเช็คลิสต์</a>
            ...
            <div class="flex items-center pl-5">
                @if(!empty($projects->status))
                    ...
                    @if($projects->status == '3')
                        <span class="mr-1 text-[14px] font-light text-white bg-green-600 rounded-md px-1 whitespace-nowrap">เสร็จสิ้น</span>
                    @endif
                    ...
                    <div class="ml-auto mr-0 inline-flex gap-2 font-medium whitespace-nowrap">
                        @if(Session::get('role') == '2')
                            ...
                            @else
                                <button class="bg-gray-400 font-medium px-1 rounded-md text-white flex items-center cursor-default">
                                    <i class='bx bxs-file-export'></i>&nbsp;EXCEL
                                </button>
                            @endif
            ...
            <div class="flex items-center pl-5 grid grid-cols-1">
                <div class="flex items-center">
                    @if(!empty($projects->status))
                        <i class='bx bx-chevrons-right text-[2rem] text-gray-700 mt-[2px]'></i>
                        @if($projects->status == '3')
                            <span class="h-fit text-[14px] mr-1 font-light text-white bg-green-600 rounded-md px-1">เสร็จสิ้น</span>
                        @endif
                ...
                <div class="inline-flex gap-2 font-medium mr-4 mt-1">
                    @if(Session::get('role') == '2')
                        ...
                        @else
                            <button class="bg-gray-400 font-medium p-1 rounded-md text-white flex items-center cursor-default">
                                <i class='bx bxs-file-export'></i>&nbsp;EXCEL
                            </button>
                        @endif
        ...
        <div class="p-5 bg-[#CDD4E8]">
            ...
            <div class="grid grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1">
                <form action="{{ Route('SearchChecklists', [Session::get('project_code'), Session::get('index')]) }}" method="get" class="col-start-3 max-lg:col-start-2 max-md:col-start-1"> 
                    ...
                    <tbody>
                        @if(sizeof($checklists_paginate))
                            @foreach($checklists_paginate as $checklist_data)
                                <tr class="bg-white text-center text-[16px] font-light border">
                                    <td class="hidden" id="checklist_id">{{ $checklist_data->id }}</td>
                                    <td scope="row" class="px-3 py-4 truncate">
                                        @php
                                            $updated_at = date("d-m-Y H:i:s A", strtotime($checklist_data->created_at));
                                            echo $updated_at;
                                        @endphp
                                    </td>
                                    <td scope="row" class="px-3 py-4 truncate">
                                    ...
                                    <td class="px-3 py-4">
                                        <div class="flex flex-wrap justify-center items-center">
                                            <div class="w-[100px] h-[80px] rounded overflow-hidden drop-shadow border">
                                    ...
                                    <td class="px-2 py-4 truncate">
                                        @if($checklist_data->status_customer == '1')
                                            <p class="flex items-center justify-center text-indigo-500">
                                                <i class='bx bx-check-square text-xl'></i>รอตรวจสอบ
                                            </p>
                                        @elseif($checklist_data->status_customer == '2')
                                            <p class="flex items-center justify-center text-teal-600">
                                    ...
                                    <td class="px-0 py-4 truncate">
                                        @if(Session::get('role') == '1')
                                            <button class="bg-yellow-500 text-white p-1 rounded-md hover:bg-yellow-600 duration-150 px-2 btnView" id="btnView"
                                        ...
                                        @else
                                            ...
                                            data-id="{{ $checklist_data->id }}" data-title="{{ $checklist_data->title }}" data-detail="{{ $checklist_data->detail }}" 
                                            data-status="{{ $checklist_data->status }}" data-image="{{ $checklist_data->image }}" onClick="ChecklistComments(this)">เรียกดู</button>
                                            <button class="bg-blue-500 text-white p-1 rounded-md hover:bg-blue-600 duration-150 px-2 btnEdit" id="btnEdit"
                                            data-id="{{ $checklist_data->id }}" data-title="{{ $checklist_data->title }}" data-detail="{{ $checklist_data->detail }}" 
                                            data-status="{{ $checklist_data->status }}" data-image="{{ $checklist_data->image }}">แก้ไข</button>
                                            <button class="bg-red-500 text-white p-1 rounded-md hover:bg-red-600 duration-150 px-2 btnDelete" id="btnDelete"
                            ...
                            @foreach($checklists_paginate as $checklist_data)
                                <div class="border border-[#CDD4E8] rounded-3xl p-2 mb-2 hover:bg-gray-100 duration-300">
                                    <div class="relative font-medium flex items-center">
                                        <div class="grid grid-cols-6 flex items-center">
                                            <div class="row-span-3 w-[100%] pr-2 relative">
                                                @if($checklist_data->image)
                                                    ...
                                                    @endphp
                                                    @if(count($images['urls']) > 1)
                                                        <span class="font-medium text-[10px] text-blue-600 bg-white rounded-full border border-gray-300 mt-[-8px] drop-shadow px-[3px] absolute right-0">+{{ count($images['urls'])-1 }}</span>
                                                    @endif
                                                    <img src="{{ URL('/uploads/' . $images['urls'][0]) }}" alt="" class="w-[80px] h-auto object-scale-down mx-auto rounded-md border p-1">
                                                @endif
                                            </div>
                                        ...
                                        @if($checklist_data->status == '1')
                                            <p class="text-yellow-600 text-[13px] font-light absolute ml-auto top-0 right-0 bg-yellow-200 rounded-2xl px-2" id="status_{{ $checklist_data->id }}-mobile">รอดำเนินการ</p>
                                        @elseif($checklist_data->status == '2')
                                            <p class="text-blue-600 text-[13px] font-light absolute ml-auto top-0 right-0 bg-blue-200 rounded-2xl px-2" id="status_{{ $checklist_data->id }}-mobile">กำลังดำเนินการ</p>
                                        @else
                                            <p class="text-teal-600 text-[13px] font-light absolute ml-auto top-0 right-0 bg-green-200 rounded-2xl px-2" id="status_{{ $checklist_data->id }}-mobile">เสร็จสิ้น</p>
                                        @endif
                                    </div>
    ...
    <div id="modalAdd" class="modal hidden fixed z-[100] pt-[100px] left-0 top-0 w-[100%] h-[100%] overflow-auto max-md:px-[10px]">
        <!-- Modal content -->
        <div class="modal-content bg-white m-auto p-[20px] rounded-md drop-shadow-xl xl:w-[40%] lg:w-[60%] md:w-[60%] sm:w-[70%] mt-[-50px] mt-[-45px]">
            <div class="flex items-center">
                ...
                <form action="#!" method="post" onsubmit="return false;" encrypt="multipart/form-data">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="title_edit" class="bg-gray-50 border border-gray-300 text-gray-700 text-md font-light rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2" placeholder="กรุณากรอกหัวข้อ" required>
                        </div>
                        <div>
        ...
        <div class="modal-content bg-white m-auto p-[20px] rounded-md drop-shadow-xl xl:w-[50%] lg:w-[60%] md:w-[70%] sm:w-[80%] mt-[-70px] max-md:mt-[-45px]">
            ...
            <div class="mt-2">
                ...
                <div class="p-3 max-md:p-0">
                    ...
                    <!-- Form Comment -->
                    <form action="#!" method="POST" onSubmit="return false;" id="commentForm" class="relative mt-9">
                        @csrf
                        ...
                        <div class="flex items-center text-md mb-2 grid grid-cols-3 max-md:grid-cols-2">
                            <div class="flex items-center col-span-2 max-md:col-span-1">
                                ...
                                <input id="username" value="&nbsp;@php if(Session::get('authen')) echo Session::get('firstname') . ' ' . Session::get('lastname') @endphp" class="outline-none cursor-text w-fit text-gray-700 font-light" readonly>
                            </div>
                            <div>
                                @if(Session::get('role') == '2')
                                    <select id="status_view" class="bg-gray-50 border border-gray-300 text-gray-700 font-light text-md rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-1" required>
                                        <option selected value="0" class="font-light text-gray-700">กำลังดำเนินการ</option>
                                        <option value="2" class="font-light text-gray-700">เสร็จสิ้น (ปิดเช็คลิสต์นี้)</option>
                                    </select>
                                @else
                                    <select id="status_view" class="hidden bg-gray-50 border border-gray-300 text-gray-700 font-light text-md rounded focus:ring-blue-500 focus:border-blue-500 block w-full p-1" required>
                                        <option selected value="0" class="font-light text-gray-700">กำลังดำเนินการ</option>
                                    </select>
                                @endif
                            </div>
                        </div>
                        <textarea id="comment" class="font-light bg-gray-50 border border-gray-300 text-gray-700 text-md rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 focus:outline-none rounded-b-sm" placeholder="เขียนความคิดเห็น..." required></textarea>
...
<script>
    ...
    // Modal Edit Comment
    function fileChosen_edit(event) {
        this.fileToDataUrl_edit(event);
    }
    function fileToDataUrl_edit(event) {
        if (! event.target.files.length){ 
            callback('');
            return
        }
        ...
        $.each(event.target.files,function(){
            ...
            reader.onload = function (e) {
                ...
                img.onload = function(){
                    ...
                    _newImgArr.push(_avatar_base64);
                    // console.log('imgAr: '+JSON.stringify(_newImgArr));
                    if (count >= limit) fileHanddle_edit(_newImgArr);
                    count++;                 
                }					
            };            
        });
    ...
    function fileHanddle_edit(src){
        $('#preview_comment_image_edit').attr('src', src);
        _image64 = src;
    }

    function ChecklistAdd(){
        // console.log('json: '+JSON.stringify(_image64));
                ...
                body:JSON.stringify(
                    {
                        ...
                        firstname: `@php echo Session::get('firstname'); @endphp`,
                        lastname: `@php echo Session::get('lastname'); @endphp`,
                        image64: _image64
                    }
                )
            })
        ...
        .then(async response => {
            ...
            if(data[1].length < 1) {
                ...
                for(let i=0; i<data[1].length; i++) {
                    ...
                    var firstname = <?php echo json_encode(Session::get('firstname')); ?>;
                    var lastname = <?php echo json_encode(Session::get('lastname')); ?>;
                    fullName = firstname + ' ' + lastname;
                    if(data[0].status == '3' || <?php echo Session::get('role'); ?> == '1' &&
                        data[1][i].username != fullName) {
                        _newBox.find('.div-edit').remove();
                        _newBox.find('.div-delete').remove();
                        _newBox.find('.tag-hr-1').remove();
                        _newBox.find('.tag-hr-2').remove();
        ...
        if(isProcessing === false && 
            ...
            if(document.getElementById("status_view").value == '2') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(async response => {
                            ...
                            if(<?php echo Session::get('role'); ?> == '1') {
                                _newBox.find('.btnEditComment').remove();
                                _newBox.find('.btnDeleteComment').remove();
                                _newBox.find('.tag-hr-1').remove();
                                _newBox.find('.tag-hr-2').remove();
                            }
                            ...
                            if(document.getElementById("status_view").value == '2') {
                                // formContent.removeClass('hidden');
                                formContent.addClass('hidden');
                                $('#status_'+document.getElementById("id").value).text('เสร็จสิ้น').removeClass('text-yellow-500').addClass('text-teal-500');
                                $('#status_'+document.getElementById("id").value+'-mobile').text('เสร็จสิ้น').removeClass('text-yellow-600 bg-blue-200').addClass('text-teal-600 bg-green-200');
                            }
                        })
                        .catch((er) => {
                            console.log('Error' + er);
                ...
                .then(async response => {
                    ...
                    var lastname = <?php echo json_encode(Session::get('lastname')); ?>;
                    fullName = firstname + ' ' + lastname;
                    if(<?php echo Session::get('role'); ?> == '1' && data.username != fullName) {
                        _newBox.find('.btnEditComment').remove();
                        _newBox.find('.btnDeleteComment').remove();
                        _newBox.find('.tag-hr-1').remove();
                        _newBox.find('.tag-hr-2').remove();
